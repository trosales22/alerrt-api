...
if($session_role == "ADMIN"){
  $queryForGettingStatusOfReport = "SELECT count(case when A.TopicStatus='Pending' then 1 end) as pending_count,count(case when A.TopicStatus='Ongoing' then 1 end) as ongoing_count,count(case when A.TopicStatus='Resolved' then 1 end) as resolved_count FROM tblposts A WHERE A.TopicAgencyID='$session_agency'";
	...
	$queryForGettingAllAgency = "SELECT AgencyID,AgencyCaption FROM tblagency WHERE AgencyStatus='Active' ORDER BY AgencyCaption ASC";
	$resultForGettingAllAgency = mysqli_query($con,$queryForGettingAllAgency);
	$numrowsForGettingAllAgency = mysqli_num_rows($resultForGettingAllAgency);

	if($numrowsForGettingAllAgency > 0){
		while($rowForGettingAllAgency = mysqli_fetch_assoc($resultForGettingAllAgency)){
			$agencyID = $rowForGettingAllAgency['AgencyID'];
			$agencyName = $rowForGettingAllAgency['AgencyCaption'];

			$queryForGettingCountOfResolvedReports = "SELECT COUNT(*) AS resolved_report_count FROM tblposts WHERE TopicAgencyID='$agencyID' AND TopicStatus='Resolved'";
			$resultForGettingCountOfResolvedReports = mysqli_query($con,$queryForGettingCountOfResolvedReports);
			$numrowsForGettingCountOfResolvedReports = mysqli_num_rows($resultForGettingCountOfResolvedReports);
			if($numrowsForGettingCountOfResolvedReports > 0){
				while($rowForGettingCountOfResolvedReports = mysqli_fetch_assoc($resultForGettingCountOfResolvedReports)){
					$count = $rowForGettingCountOfResolvedReports['resolved_report_count'];
					array_push($dataPoints, array("label"=>$agencyName, "y"=>$count));
				}
			}
		}
	}
...
<body class="">
  ...
  <script src="assets/canvasjs-2.3/canvasjs.min.js"></script>
